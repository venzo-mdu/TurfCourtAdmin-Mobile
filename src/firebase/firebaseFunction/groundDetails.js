...
export const getgroundDataForOwner = async uid => {
  try {
    ...
    return filterdata?.map(item => {
      const groundCourts = courts.filter(c => c.ground_id === item.ground_id);
...
export const getEventdetailsByArenas = async ({
  groundIds,
...
export const removefavGround = async (uid, ground_id) => {
  try {
    const userdata = await userData(uid);
    if (userdata) {
      userdata.favarote = userdata.favarote.filter(item => item !== ground_id);
    }
